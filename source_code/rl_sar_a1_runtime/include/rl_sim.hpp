...
private:
    ...
    bool auto_start_locomotion = false;
    bool auto_start_complete = false;
    bool has_cmd_vel = false;
    double cmd_vel_timeout_sec = 0.25;
    double min_vx = -0.25;
    double max_vx = 0.45;
    double min_vy = 0.0;
    double max_vy = 0.0;
    double max_wz = 0.70;
    double cmd_vel_yaw_scale = 1.0;
    double acc_lim_x = 0.60;
    double acc_lim_y = 0.0;
    double acc_lim_wz = 1.20;
    rclcpp::Time last_cmd_vel_time;
    rclcpp::Time last_nav_filter_time;
    std::array<float, 3> filtered_nav_cmd = {0.0f, 0.0f, 0.0f};
    void LoadNavigationParameters();
    void UpdateAutoStart();
    std::vector<float> GetNavigationCommand();
#endif
