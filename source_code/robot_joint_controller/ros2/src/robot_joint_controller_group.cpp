...
RobotJointControllerGroup::RobotJointControllerGroup()
    : controller_interface::ControllerInterface(),
        ...
        controller_stamped_state_publisher_(nullptr)
{
...
CallbackReturn RobotJointControllerGroup::on_configure(const rclcpp_lifecycle::State &previous_state)
{
    ...
    imu_subscriber_ = get_node()->create_subscription<sensor_msgs::msg::Imu>(
        ...
        std::bind(&RobotJointControllerGroup::ImuCallback, this, std::placeholders::_1));
    ...
    controller_stamped_state_publisher_ =
        std::make_shared<realtime_tools::RealtimePublisher<robot_msgs::msg::StampedRobotState>>(
            get_node()->create_publisher<robot_msgs::msg::StampedRobotState>(
                /*name_space_ + */"~/state_stamped", rclcpp::SystemDefaultsQoS()));
...
controller_interface::return_type RobotJointControllerGroup::update()
{
    ...
    UpdateFunc(current_time, period_seconds);
    return controller_interface::return_type::OK;
}
...
controller_interface::return_type RobotJointControllerGroup::update(const rclcpp::Time &time, const rclcpp::Duration &period)
{
    ...
    last_command_ = *(joint_commands);
    UpdateFunc(time, period_seconds);
    return controller_interface::return_type::OK;
}
#endif

void RobotJointControllerGroup::UpdateFunc(const rclcpp::Time &time, const double &period_seconds)
{
    ...
    if (controller_stamped_state_publisher_ && controller_stamped_state_publisher_->trylock())
    {
        controller_stamped_state_publisher_->msg_.header.stamp = time;
        controller_stamped_state_publisher_->msg_.header.frame_id = "base";
        controller_stamped_state_publisher_->msg_.motor_state = last_state_.motor_state;
        {
            std::lock_guard<std::mutex> lock(imu_mutex_);
            if (has_latest_imu_)
            {
                controller_stamped_state_publisher_->msg_.imu = latest_imu_;
                controller_stamped_state_publisher_->msg_.imu.header.stamp = time;
            }
            else
            {
                controller_stamped_state_publisher_->msg_.imu = sensor_msgs::msg::Imu();
                controller_stamped_state_publisher_->msg_.imu.header.stamp = time;
                controller_stamped_state_publisher_->msg_.imu.header.frame_id = "imu_link";
                controller_stamped_state_publisher_->msg_.imu.orientation.w = 1.0;
            }
        }
        controller_stamped_state_publisher_->unlockAndPublish();
    }
}
...
void RobotJointControllerGroup::ImuCallback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
    std::lock_guard<std::mutex> lock(imu_mutex_);
    latest_imu_ = *msg;
    has_latest_imu_ = true;
}
