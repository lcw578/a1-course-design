...
- 平均真值目标点误差：`0.177 m`
- 平均 `/odom` 目标点误差：`0.177 m`
- `/odom` 相对 `/odom_gt` 的位置 RMSE：`0.001 m`
- `cmd_vel` 非零比例：`0.878`

逐目标结果：

- `straight_1p5`: 状态 `SUCCEEDED`，真值目标点误差 `0.182 m`，Nav2 使用 `/odom` 的目标点误差 `0.182 m`，真值路径长度 `1.293 m`，耗时 `7.41 s`。
- `left_turn_2p4`: 状态 `SUCCEEDED`，真值目标点误差 `0.173 m`，Nav2 使用 `/odom` 的目标点误差 `0.173 m`，真值路径长度 `0.958 m`，耗时 `5.96 s`。
- `straight_after_turn_3p3`: 状态 `SUCCEEDED`，真值目标点误差 `0.175 m`，Nav2 使用 `/odom` 的目标点误差 `0.175 m`，真值路径长度 `0.950 m`，耗时 `6.46 s`。
- `right_correction_4p1`: 状态 `SUCCEEDED`，真值目标点误差 `0.177 m`，Nav2 使用 `/odom` 的目标点误差 `0.177 m`，真值路径长度 `0.842 m`，耗时 `5.66 s`。
