...
- 平均真值目标点误差：`0.295 m`
- 平均 AMCL 目标点误差：`0.330 m`
- `/odom` 相对 `/odom_gt` 的位置 RMSE：`0.696 m`
- `map->base_footprint` 相对 `/odom_gt` 的位置 RMSE：`0.648 m`
- `cmd_vel` 非零比例：`0.946`

逐目标结果：

- `goal_1m`: 状态 `SUCCEEDED`，真值目标点误差 `0.093 m`，AMCL 目标点误差 `0.049 m`，真值路径长度 `0.859 m`，耗时 `4.66 s`。
- `goal_2m`: 状态 `SUCCEEDED`，真值目标点误差 `0.133 m`，AMCL 目标点误差 `0.118 m`，真值路径长度 `0.950 m`，耗时 `5.51 s`。
- `goal_left`: 状态 `SUCCEEDED`，真值目标点误差 `0.659 m`，AMCL 目标点误差 `0.823 m`，真值路径长度 `5.610 m`，耗时 `27.46 s`。
